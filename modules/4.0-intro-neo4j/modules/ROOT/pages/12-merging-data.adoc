...
At the end of this module, you will be able to write Cypher statements to:
[square]
...
When you specify _nodeProperties_ for `MERGE`, you only use properties that satisfy some sort of uniqueness constraint.
...
[.notes]
--
Using `MERGE` to create relationships is expensive and you only do it when you need to ensure that a relationship is unique and you are [underline]#not# sure if it already exists.
...
That's why in most cases you first `MERGE` your nodes and then your relationship afterwards.
--
...
[.statement]
Given this `MERGE` clause, what is the most important thing you will make sure of?
