...
When you inspect a subset of the data, you must be able to determine what transformations will be required.
...
Note: They will do the import in the hands-on exercise at the end of the lesson.
endif::[]
...
To do this, you must create constraints for the data.

For this movie data in the CSV files, we want to ensure that a Movie node is unique as well as a Person node.
The IDs in the CSV files ideally are unique, but create the constraints in the graph to ensure that this will be true when data is imported.
...
Create additional indexes in the graph after the data is loaded.
--
...
[.statement]
What must you add to the graph before you import using `LOAD CSV`?
