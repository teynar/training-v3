...
|PageRank measures importance in comparison to other nodes using an iterative process to update ranks.
 It starts by assigning values to nodes as 1/n (n is the total number of nodes linked to it) and value to relationships as that nodes value/number of its outgoing links.
|===
{set:cellbgcolor!}
endif::[]
