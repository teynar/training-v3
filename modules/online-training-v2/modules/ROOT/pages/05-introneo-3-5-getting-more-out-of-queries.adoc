...
Only perform these types of queries if you know the data well and the implications of doing the query.
...
When you use the `shortestPath()` function, the query editor will show a warning that this type of query could potentially run for a long time. You must heed the warning, especially for large graphs. Read the _Graph Algorithms_ documentation about the shortest path algorithm.
...
Here is an example Cypher to retrieve all actors from the graph, and if they have a value for _born_, calculate the _age_ value.
