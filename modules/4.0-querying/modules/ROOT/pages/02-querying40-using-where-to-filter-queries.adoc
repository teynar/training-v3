...
[IMPORTANT]
In Neo4j 4.3, `exists()` has been deprecated and may not work in future releases, but it still works in 4.3. In Neo4j 4.3 and later you can use `AND m.tagline IS NOT NULL`
...
[NOTE]
The  syntax in 4.3 or when exists() is longer available for testing the existence of a pattern in the graph is:
`WHERE ( (p)-[:DIRECTED]->(m) ) = []`
