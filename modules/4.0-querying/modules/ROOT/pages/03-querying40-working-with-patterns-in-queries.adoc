...
[.notes]
--
The actors _Laurence Fishburne_, _Carrie-Anne Moss_, _Emil Eifrem_ (and of course _Hugo Weaving_ and _Keanu Reeves_) do not appear in the results list, because these actors were in the same movie (_The Matrix_) as _Hugo Weaving_ and _Keanu Reeves_.
--
...
That is why the _Meg Ryan_ node is not retrieved in the _other_ node retrievals. All other nodes of people who acted in that same movie are retrieved.
...
[.notes]
--
Some actor relationships have data for the roles property or summary property of the relationship. Note that in this text, the name of the relationship is not shown, but it is in the graph visualization.
...
- [x] Add a `MATCH` clause at the beginning of the second line.
