...
1. Using the admin token, create a new user. 
2. Next, create a JWT token for this user using jwt.io. Use this token to create an order for this user. Be sure to include some books in the order! 
3. Next, add a review for the book purchased by this user. Finally, write a query to view the customer's details, including their order history and their reviews.
...
In this lesson, we explored how to add authorization rules . In the next lesson we explore generating GraphQL API using the Neo4j GraphQL Library.