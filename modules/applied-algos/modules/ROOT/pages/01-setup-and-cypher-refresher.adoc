...
.. Confirm that you can now view data in the database by selecting a name in the drop down (initially selected with Dolores). Data will be retrieved for each user.
...
NOTE: The creation of the Yelp Sandbox has changed and you must use the link provided above for creating the Yelp Sandbox, as well as using the *Connection Details* information for the sandbox.
