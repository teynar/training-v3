...
Where The result returned will be:
...
What happens when you add the constraint? This is what you will see.
...
What happens when you execute the statement? The result returned will be:
...
[source, cypher]
----
CREATE CONSTRAINT ActedInRolesExistConstraint ON ()-[r:ACTED_IN]-() ASSERT r.roles IS NOT NULL
----
...
What happens when you execute the statement? Here is what you will see:
